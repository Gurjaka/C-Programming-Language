...
{
    unsigned mask = ((1U << n) - 1) << (p - n + 1);         // Create mask for the n bits at position p
    unsigned y_bits = (y & ((1U << n) - 1)) << (p - n + 1); // Extract rightmost n bits from y and shift to position p
