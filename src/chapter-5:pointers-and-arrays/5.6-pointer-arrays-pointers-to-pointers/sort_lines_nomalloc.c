...
int getLine(char *buffer, int limit) {
  int character, index;
  for (index = 0;
       index < limit - 1 && (character = getchar()) != EOF && character != '\n';
       ++index)
...
int readLines(char *linePointers[], int maxLines) {
  ...
  while (numberOfLines < maxLines &&
         (lineLength = getLine(lineStorage[numberOfLines], MAX_LINE_LENGTH)) >
             0) {
    lineStorage[numberOfLines][lineLength - 1] = '\0'; /* Remove newline */
    linePointers[numberOfLines] =
        lineStorage[numberOfLines]; /* Store pointer */
    numberOfLines++;
  }
